SELECT
    ...
    (SELECT MIN(CHANGE_BUSINESS_DATE) FROM  ICS_GENERAL_ACCOUNT_STATUS_H WHERE ID_ACCOUNT_STATUS_OLD IS NULL AND ID_GENERAL_ACCOUNT = gralAcc.ID_GENERAL_ACCOUNT) IssueDate,
    ...
    '' as CreditAmountDate,
    accProperty.EARNS_LOYALTY_POINTS AS LoyaltyPoints
FROM
...
-- Habilitado disposición de efectivo
-- Modalidad de pago anticipado

select * from C_ORGANIZATION;
select * from IAC_GENERAL_ACCOUNT;
inner join isuser_bracn -> organization_id y obtener el external id
